...
def stock_marca(marca):
    ...
    for modelos in stock_modelos:
        print(
            f"La marca {marca} tiene de stock el modelo: {modelos} con stock de: {stock[modelos][1]}")
...
def busqueda_por_precio(rango_min, rango_max):
    ...
    for m, mar in productos.items():
        if m in modelos_dentro_rango:
            print(
                f"El modelo: {m} de marca: {mar[0]} de encuentra dentro del rango.")
...
def actualizar_precio(modelo):
    while True:
        try:
            ...
            actualizar = input(
                "¿Desea actualizar otro precio de notebook? (SI/NO)").upper()
            if actualizar == "SI":
...
""")
            ...
            if elec < 1 or elec > 4:
                ...
                if elec == 1:
                    marca = input(
                        "Ingrese la marca que desea consultar su stock: ").lower()
                    stock_marca(marca)
                elif elec == 2:
                    while True:
                        try:
